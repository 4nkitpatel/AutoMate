...
def translate(data, Tolanguage):
    ...
    for key, value in languages.items():
        if Tolanguage.lower() == value:
            translated = translator.translate(data, dest=key)
            # 'ते कसे चालले होते?'
            TextToSpeech.say("Your converted Text is Ready")
            eel.printAgentDom(translated.text)
            return

    eel.printAgentDom("I can't recognize the language")
    TextToSpeech.say("I can't recognize the language")
